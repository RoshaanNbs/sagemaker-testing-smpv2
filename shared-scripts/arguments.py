...
def parse_args():  # pylint: disable=too-many-statements
    ...
    ckpt_grp.add_argument(
        ...
        nargs="+",
        type=str,
        default=["s3://nbs-sharding/tubitak-model-output-5GB-SUREN-1/checkpoints"],
    ...
    ckpt_grp.add_argument(
        "--model_dir",
        type=str,
        default="s3://nbs-sharding/tubitak-model-output-5GB-SUREN-1/",
        help="If not passed, saves it to checkpoint_dir/model. Only saved when save_final_model is 1",
    )
    ckpt_grp.add_argument("--save_final_model", type=int, default=1)
