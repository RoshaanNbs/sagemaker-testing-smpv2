...
# Ensure these functions iterate over the writers list
import pynvml

# Initialize NVML
pynvml.nvmlInit()

def log_gpu_utilization(writers, device, step):
    handle = pynvml.nvmlDeviceGetHandleByIndex(device)
    gpu_utilization = pynvml.nvmlDeviceGetUtilizationRates(handle).gpu
    for writer in writers:
        writer.add_scalar("GPU Utilization", gpu_utilization, step)

def memory_status(tag, writers, step):
    mem_bytes = torch.cuda.max_memory_allocated()
    mem_gb = mem_bytes / (1024 ** 3)
    for writer in writers:
        writer.add_scalar(f"Memory Usage {tag}", mem_gb, step)

def memory_status_cpu(tag, writers, step):
    import psutil
    mem = psutil.virtual_memory()
    mem_gb = mem.used / (1024 ** 3)
    for writer in writers:
        writer.add_scalar(f"CPU Memory Usage {tag}", mem_gb, step)

def log_and_write_eval_metrics(writers, step, val_loss, val_ppl):
    for writer in writers:
        writer.add_scalar("Validation Loss", val_loss, step)
        writer.add_scalar("Perplexity", val_ppl, step)

def log_and_write_train_metrics(writers, step, loss, throughput, tflops, lr, grad_norm):
    for writer in writers:
        writer.add_scalar("Training Loss", loss, step)
        writer.add_scalar("Throughput", throughput, step)
        writer.add_scalar("TFLOPS", tflops, step)
        writer.add_scalar("Learning Rate", lr, step)
        writer.add_scalar("Gradient Norm", grad_norm, step)

def write_metrics_train_step(writers, step, loss, throughput, tflops, lr, grad_norm):
    log_and_write_train_metrics(writers, step, loss, throughput, tflops, lr, grad_norm)
    memory_status("After step", writers, step)
    memory_status_cpu("After step", writers, step)
    device = torch.cuda.current_device()
    log_gpu_utilization(writers, device, step)
...
def create_model(args, model_config, dtype, pretrained_model_weights=None):
    ...
    # Set dtype based on args.bf16 and args.fp16 
    # Changed 6/26/2024
...
def get_model_config(args):
    ...
    elif "llama_v2" in args.model_type or "llama_v3" in args.model_type:
