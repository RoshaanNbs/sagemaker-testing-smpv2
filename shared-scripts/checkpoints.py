...
def compute_stats_of_metric(metric: float, key: str, group: Optional[Any] = None):
    ...
    if dist.get_rank() == 0:
        if len(times) > 1:
            ...
            stdev = 0.0  # Handle single data point case
...
def _save_sharded(model, optimizer, scheduler, user_content, save_dir: str, checkpointing_pg_metadata):
    ...
    if dist.get_rank() == 0:
        ...
        logger.info(f"Keys in state_dict['model']: {list(state_dict['model'].keys())}")
    ...
    with warnings.catch_warnings():
        ...
        if action_rank:
            ...
            for name, tensor in state_dict["model"].items():
                if isinstance(tensor, dict): # Check if it's a nested dictionary
                    for inner_name, inner_tensor in tensor.items():
                        if isinstance(inner_tensor, torch.Tensor):
                            logger.info(f"Tensor {name}.{inner_name} shape: {inner_tensor.shape}")
                        else:
                            logger.info(f"{name}.{inner_name} is not a tensor, it is of type {type(inner_tensor)}, skipping shape log")
                elif isinstance(tensor, torch.Tensor):
                    logger.info(f"Tensor {name} shape: {tensor.shape}")
                else:
                    logger.info(f"{name} is not a tensor, it is of type {type(tensor)}, skipping shape log")
...
def _save_full(model, save_dir: str, user_content: Dict):
    """Save FSDP checkpoint: Without process groups."""
    if dist.get_rank() == 0:
    ...
    with FSDP.state_dict_type(
        ...
        # Add debug information to check tensor shapes
        for name, tensor in state_dict.items():
            logger.info(f"Tensor {name} shape: {tensor.shape}")

        # Add torch.cuda.empty_cache() before saving
        torch.cuda.empty_cache()
...
def upload_files_to_s3(save_dir, bucket, bucketdir):
    s3_client = boto3.client("s3")
    for root, dirs, files in os.walk(save_dir):
        for file in files:
            fpath = os.path.join(root, file)
            rel_path = os.path.relpath(fpath, save_dir)
            bucketobj = os.path.join(bucketdir, rel_path)
...
def save_checkpoint(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    if is_s3_source(root_dir):
        ...
        upload_files_to_s3(save_dir, bucket, bucketdir)
